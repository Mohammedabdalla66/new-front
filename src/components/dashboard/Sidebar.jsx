...
const Sidebar = ({ isOpen, onClose, currentPath }) => {
  ...
  // Listen for notification clearing events from pages
  useEffect(() => {
    const handleClearMessagesBadge = () => {
      setUnreadMessages(0);
    };

    const handleClearRequestsBadge = () => {
      // Clear the proposals badge by resetting proposals count
      setStats(prev => prev ? { ...prev, proposals: { total: 0, pending: 0, active: 0, accepted: 0 } } : null);
    };

    const handleRefreshSidebarBadges = () => {
      // Refresh all sidebar badge counts
      const userRole = authUser?.role || userData?.role;
      if (userRole && !loadingMessagesRef.current) {
        loadingMessagesRef.current = true;
        const apiCall = userRole === 'client' 
          ? messagesAPI.listClientConversations()
      ...
      // Also refresh stats for service providers
      if (userRole === 'serviceProvider' && !loadingStatsRef.current) {
        loadingStatsRef.current = true;
        serviceProviderAPI.getDashboardStats()
          .then(response => {
            if (response.data.success) {
    ...
    window.addEventListener('clearMessagesBadge', handleClearMessagesBadge);
    window.addEventListener('clearRequestsBadge', handleClearRequestsBadge);
    window.addEventListener('refreshSidebarBadges', handleRefreshSidebarBadges);

    return () => {
      window.removeEventListener('clearMessagesBadge', handleClearMessagesBadge);
      window.removeEventListener('clearRequestsBadge', handleClearRequestsBadge);
      window.removeEventListener('refreshSidebarBadges', handleRefreshSidebarBadges);
    };
  }, [authUser, userData]);
