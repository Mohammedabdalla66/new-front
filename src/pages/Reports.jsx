...
const Reports = () => {
  ...
  const refundRate =
    totalTransactions === 0
      ? 0
      : Math.round(
          (filtered.reduce((s, r) => s + r.refunds, 0) / totalTransactions) *
            100
        );
  ...
  useEffect(() => {
    ...
    else if (filters.reportType === "Clients Activity")
      setSummary(
        mockSummary.map((r) => ({
          ...r,
          transactions: Math.round(r.transactions * 1.1),
        }))
      );
    else
      setSummary(mockSummary.map((r) => ({ ...r, revenue: r.revenue * 1.05 })));
  }, [filters.reportType]);
  ...
  const onGenerate = () => {
    ...
    setToast({
      ...
      description: "KPIs and charts updated for the selected filters.",
      variant: "success",
    });
  ...
  const onExport = (format) => {
    if (format === "CSV") {
      ...
      const blob = new Blob([`Mock ${format} export`], {
  ...
  const triggerExportWithToast = (format) => {
    ...
    setToast({
      ...
      description: `Report exported successfully as ${format}.`,
      variant: "success",
    });
    setTimeout(() => setToast((t) => ({ ...t, open: false })), 2000);
  };
  ...
  const saveSchedule = (s) => {
    ...
    if (s.id) {
      setSchedules((prev) =>
        prev.map((x) =>
          x.id === s.id ? { ...x, ...s, nextDelivery: next } : x
        )
      );
    } else {
            ...
            <TableCell colSpan={7}>
              <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="text-neutral-900 dark:text-neutral-100">
                    ${(row.revenue / Math.max(1, row.transactions)).toFixed(2)}
                  </div>
                </div>
                <div>
