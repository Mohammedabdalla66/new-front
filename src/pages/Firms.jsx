...
const Firms = () => {
  ...
  useEffect(() => {
    const loadFirms = async () => {
      try {
        setLoading(true);
        const response = await adminAPI.listUsers({ role: 'serviceProvider' });
        const users = response.data || [];
        setFirms(users.map(user => ({
          id: user._id || user.id,
          name: user.name || 'N/A',
          email: user.email || '',
          ...
          logo: (user.name || 'N/A').split(' ').map(word => word[0]).join('').toUpperCase().slice(0, 2),
        })));
      } catch (error) {
        ...
        setToast({
          open: true,
          title: 'Error',
          description: error?.response?.data?.message || 'Failed to load firms',
          variant: 'destructive'
        });
  ...
  const updateFirmStatus = async (id, status) => {
    try {
      ...
      // Keep modal data in sync if it's the same firm
      setSelectedFirm((prev) => (prev && prev.id === id ? { ...prev, status } : prev));
      return true;
    } catch (error) {
      ...
      setToast({
        open: true,
        title: 'Error',
        description: error?.response?.data?.message || 'Failed to update firm status',
        variant: 'destructive'
      });
      setTimeout(() => setToast((t) => ({ ...t, open: false })), 3000);
      return false;
  ...
  const approveFirm = async (firm) => {
    const success = await updateFirmStatus(firm.id, 'Active');
    if (success) {
      setToast({ open: true, title: 'Firm approved', description: `${firm.name} is now Active and can sign in.`, variant: 'success' });
      setTimeout(() => setToast((t) => ({ ...t, open: false })), 2500);
      setDialogOpen(false);
      // Reload firms to get updated data
      const response = await adminAPI.listUsers({ role: 'serviceProvider' });
      const users = response.data || [];
      setFirms(users.map(user => ({
        id: user._id || user.id,
  ...
  const rejectFirm = async (firm) => {
    const success = await updateFirmStatus(firm.id, 'Pending');
    if (success) {
      setToast({ open: true, title: 'Firm deactivated', description: `${firm.name} is now Pending and cannot sign in.`, variant: 'success' });
      setTimeout(() => setToast((t) => ({ ...t, open: false })), 2500);
      setDialogOpen(false);
      // Reload firms to get updated data
      const response = await adminAPI.listUsers({ role: 'serviceProvider' });
  ...
  }, [firms, searchQuery, statusFilter]);
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  {selectedFirm.status === 'Pending' ? (
                    <>
                      ...
                      <Button variant="destructive" onClick={() => rejectFirm(selectedFirm)}>
                        <XCircle className="w-4 h-4 mr-2" />
                        Reject
                      </Button>
                      <Button onClick={() => approveFirm(selectedFirm)}>
